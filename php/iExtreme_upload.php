...
$user_main_wd = getcwd()."/../model/user_pred_log";
...
#----------------------------
# pre-processing
# step 1 pred orf with prodigal, file name: user_pred.cds
$user_genome = $user_raw_file_path;
$command = sprintf("python3 ../python/exec_prodigal.py %s %s", $user_genome, $user_cds_db);
exec($command);

$input_cds_path = sprintf("%s/user_pred.cds",$user_cds_db);
$outfile_path = $raw_file_path."/"."user_process.tsv";
$command = sprintf("python3 ../python/preprocess_data.py %s %s", $input_cds_path, $outfile_path);
exec($command);
